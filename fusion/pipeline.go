...
func (p *FusionPipeline) outOfBounds(x, y float64) bool {
	...
	return x < p.bounds.minX || x > p.bounds.maxX || y < p.bounds.minY || y > p.bounds.maxY
}

func (p *FusionPipeline) clampToBounds(x, y float64) (float64, float64) {
	if !p.bounds.has {
		return x, y
	}
	return clamp(x, p.bounds.minX, p.bounds.maxX), clamp(y, p.bounds.minY, p.bounds.maxY)
}

func (p *FusionPipeline) extendBounds(x, y float64) {
	...
	if x-MapMargin < p.bounds.minX {
		p.bounds.minX = x - MapMargin
	}
	if x+MapMargin > p.bounds.maxX {
		p.bounds.maxX = x + MapMargin
	}
	if y-MapMargin < p.bounds.minY {
		p.bounds.minY = y - MapMargin
	}
	if y+MapMargin > p.bounds.maxY {
		p.bounds.maxY = y + MapMargin
	}
}
...
func (p *FusionPipeline) Process(tsMs int64, tagID int, bleMeas []BLEMeas, twrMeas []TWRMeas, tagHeight float64) FusionResult {
	...
	*p.lastTS = tsMs
	...
	if p.looseFusor.Latest(&looseEst) {
		...
		// Prefer graph smoother output when available
		if gx, gy, ok := p.graph.Latest(); ok {
			outX, outY = gx, gy
		}
	}
	...
	if p.hasLastGood && p.lastGoodTs != nil {
		...
		if dtg > 0 {
			step := math.Hypot(outX-p.lastGoodPos[0], outY-p.lastGoodPos[1])
			allowed := KinematicSpeedMax*dtg + KinematicSlack
			if step > allowed {
				p.resetFilters()
				p.lastTS = new(int64)
				*p.lastTS = tsMs
...
// computeMapBounds builds a loose bounding box around anchors/dimensions (meters) with padding.
func computeMapBounds(anchors map[int]Anchor, dimMap map[int][]DimMat, beaconDims map[int][]DimMat) mapBounds {
	...
	addMats := func(mats map[int][]DimMat) {
		for _, lst := range mats {
			for _, mat := range lst {
				for _, pt := range mat {
					if len(pt) >= 2 {
						push(pt[0], pt[1])
	...
	if math.IsInf(minX, 1) || math.IsInf(maxX, -1) || math.IsInf(minY, 1) || math.IsInf(maxY, -1) {
		return mapBounds{}
	}
	margin := MapMargin
	if maxX-minX < 1.0 {
